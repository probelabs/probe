...
"#;
    ...
    assert!(has_symbol_signatures, "At least one result should have a symbol signature");
    ...
    let has_symbol_signatures = results_no_symbols.results.iter().any(|r| r.symbol_signature.is_some());
    assert!(!has_symbol_signatures, "No results should have symbol signatures when flag is disabled");
}
...
"#;
    ...
    let python_symbols: Vec<_> = results.results.iter()
        .filter_map(|r| r.symbol_signature.as_ref())
        .collect();
    
    assert!(!python_symbols.is_empty(), "Should have Python symbol signatures");
    
    // Check that we have recognizable Python function signatures
    let has_python_function = python_symbols.iter()
        .any(|sig| sig.contains("def ") && sig.contains("(") && sig.contains(")"));
    assert!(has_python_function, "Should have Python function signatures");
}