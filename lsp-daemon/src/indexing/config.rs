...
impl IndexingConfig {
    ...
    pub fn from_env() -> Result<Self> {
        ...
        if let Ok(value) = std::env::var("PROBE_INDEX_DEPTH") {
            config.default_depth = value.parse()
        ...
        if let Ok(value) = std::env::var("PROBE_INDEX_WORKERS") {
            ...
            if workers > 0 && workers <= 64 {
                ...
                return Err(anyhow!("PROBE_INDEX_WORKERS must be between 1 and 64, got {}", workers));
            }
        }
        
        if let Ok(value) = std::env::var("PROBE_INDEX_MEMORY_MB") {
            config.memory_budget_mb = value.parse()
        ...
        if let Ok(value) = std::env::var("PROBE_INDEX_PRIORITY_LANGS") {
            let languages: Result<Vec<Language>, _> = value
                .split(',')
                .map(|s| s.trim().parse())
                .collect();
            config.priority_languages = languages
                .context("Invalid language in PROBE_INDEX_PRIORITY_LANGS")?;
        }
        ...
        if let Ok(value) = std::env::var("PROBE_INDEX_DISABLED_LANGS") {
            let languages: Result<Vec<Language>, _> = value
                ...
                .map(|s| s.trim().parse())
                .collect();
            config.disabled_languages = languages
                .context("Invalid language in PROBE_INDEX_DISABLED_LANGS")?;
        }
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.max_workers > 64 {
            return Err(anyhow!("max_workers should not exceed 64 for performance reasons"));
        }
        
        if self.memory_pressure_threshold < 0.0 || self.memory_pressure_threshold > 1.0 {
            return Err(anyhow!("memory_pressure_threshold must be between 0.0 and 1.0"));
        }
        ...
        if self.file_processing_timeout_ms < 1000 {
            warn!("file_processing_timeout_ms is very low ({}ms), may cause timeouts", self.file_processing_timeout_ms);
        }
    ...
    pub fn for_language(&self, language: Language) -> EffectiveConfig {
        ...
        EffectiveConfig {
            ...
            cache_strategy: language_config
                .map(|c| c.cache_strategy.clone())
                .unwrap_or(if self.persist_cache {
    ...
    pub fn to_protocol_config(&self) -> crate::protocol::IndexingConfig {
        crate::protocol::IndexingConfig {
            ...
            languages: self.priority_languages.iter().map(|l| format!("{:?}", l)).collect(),
    ...
    pub fn from_protocol_config(protocol: &crate::protocol::IndexingConfig) -> Self {
        ...
        if !protocol.languages.is_empty() {
            config.priority_languages = protocol.languages
                .iter()
...
impl IndexingFeatures {
    ...
    pub fn from_env() -> Result<Self> {
        ...
        if let Ok(value) = std::env::var("PROBE_INDEX_EXTRACT_ERROR_HANDLING") {
            features.extract_error_handling = parse_bool_env(&value, "PROBE_INDEX_EXTRACT_ERROR_HANDLING")?;
        }
        ...
        if let Ok(value) = std::env::var("PROBE_INDEX_EXTRACT_API_ENDPOINTS") {
            features.extract_api_endpoints = parse_bool_env(&value, "PROBE_INDEX_EXTRACT_API_ENDPOINTS")?;
        }
    ...
    pub fn validate(&self, language: &Language) -> Result<()> {
        if let Some(workers) = self.max_workers {
            if workers == 0 || workers > 32 {
                return Err(anyhow!("max_workers for {:?} must be between 1 and 32", language));
            }
        }
...
fn load_language_configs_from_env() -> Result<HashMap<Language, LanguageIndexConfig>> {
    ...
    for language in [
        ...
        if let Ok(value) = std::env::var(&format!("PROBE_INDEX_{}_ENABLED", lang_str)) {
            config.enabled = Some(parse_bool_env(&value, &format!("PROBE_INDEX_{}_ENABLED", lang_str))?);
            has_config = true;
        }
        
        if let Ok(value) = std::env::var(&format!("PROBE_INDEX_{}_WORKERS", lang_str)) {
            config.max_workers = Some(value.parse()
                .context(format!("Invalid value for PROBE_INDEX_{}_WORKERS", lang_str))?);
            has_config = true;
        }
        
        if let Ok(value) = std::env::var(&format!("PROBE_INDEX_{}_MEMORY_MB", lang_str)) {
            config.memory_budget_mb = Some(value.parse()
                .context(format!("Invalid value for PROBE_INDEX_{}_MEMORY_MB", lang_str))?);
            has_config = true;
        }
        
        if let Ok(value) = std::env::var(&format!("PROBE_INDEX_{}_TIMEOUT_MS", lang_str)) {
            config.timeout_ms = Some(value.parse()
                .context(format!("Invalid value for PROBE_INDEX_{}_TIMEOUT_MS", lang_str))?);
            has_config = true;
        }
        
        if let Ok(value) = std::env::var(&format!("PROBE_INDEX_{}_PRIORITY", lang_str)) {
            config.priority = value.parse()
                .context(format!("Invalid value for PROBE_INDEX_{}_PRIORITY", lang_str))?;
            has_config = true;
        }
        ...
        if let Ok(value) = std::env::var(&format!("PROBE_INDEX_{}_PIPELINE", lang_str)) {
            ...
            let pipeline_enabled = parse_bool_env(&value, &format!("PROBE_INDEX_{}_PIPELINE", lang_str))?;
            if pipeline_enabled {
...
mod tests {
    ...
    fn test_effective_config() {
        ...
        base_config.language_configs.insert(Language::Rust, rust_config);
    ...
    fn test_default_extensions() {
        ...
        assert_eq!(default_extensions_for_language(Language::Python), vec!["py", "pyi"]);
        assert_eq!(default_extensions_for_language(Language::TypeScript), vec!["ts", "tsx"]);
    ...
    fn test_environment_variable_patterns() {
        ...
        assert_eq!(protocol_config.memory_budget_mb, Some(config.memory_budget_mb));
