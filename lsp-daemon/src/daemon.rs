...
use crate::protocol::{
    ...
    MessageCodec, PoolStatus, parse_call_hierarchy_from_lsp,
};
...
impl LspDaemon {
    ...
    async fn handle_request(&self, request: DaemonRequest) -> DaemonResponse {
        match request {
            DaemonRequest::Connect { client_id } => {
                DaemonResponse::Connected {
                    request_id: client_id,
                    daemon_version: env!("CARGO_PKG_VERSION").to_string(),
                }
            }
            
            DaemonRequest::CallHierarchy { request_id, file_path, pattern } => {
            ...
            DaemonRequest::Ping { request_id } => {
                DaemonResponse::Pong { request_id }
            }
    ...
    async fn handle_call_hierarchy(&self, file_path: &Path, pattern: &str) -> Result<CallHierarchyResult> {
        // Detect language
        ...
        // Open document
        pooled_server.server.open_document(file_path, &content).await?;
        
        // Get call hierarchy
        let result = pooled_server.server.call_hierarchy(file_path, line, column).await?;
