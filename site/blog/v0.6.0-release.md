...
- **Safe Execution**: Configurable via `allow_edit` flag with proper permission controls
...
- **Reviewdog Integration**: New `allow_suggestions` flag enables AI-powered code suggestions
...
- **probe-engineer.yml**: Dedicated workflow for engineering-focused AI assistance
...
- **Configurable Iterations**: Control AI conversation length with `MAX_TOOL_ITERATIONS`
...
- **Mandatory Path Parameters**: Improved tool definitions with required parameters
- **Better Error Handling**: More informative error messages and validation
- **Standardized Interfaces**: Consistent tool behavior across all MCP implementations
