...
fn test_search_determinism_with_user_path() {
    ...
    for i in 1..=iterations {
        ...
        println!("Iteration {}: {} lines, {} bytes, {} tokens", 
    ...
    for result in &results {
        let key = format!("{}:{}:{}", result.line_count, result.byte_count, result.token_count);
    ...
    if unique_results.len() > 1 {
        ...
        println!("Expected: All {} iterations to return identical results", iterations);
        println!("Actual: Found {} different result patterns", unique_results.len());
        ...
        for (i, result) in results.iter().enumerate().skip(1) {
            if !results_equal(first_result, result) {
                println!("\nDifference detected:");
                println!("  Iteration 1: {} lines, {} bytes, {} tokens", 
                         first_result.line_count, first_result.byte_count, first_result.token_count);
                println!("  Iteration {}: {} lines, {} bytes, {} tokens", 
                         i + 1, result.line_count, result.byte_count, result.token_count);
                break;
            }
        }
        ...
        let first_result = &results[0];
        println!("Consistent result: {} lines, {} bytes, {} tokens", 
                 first_result.line_count, first_result.byte_count, first_result.token_count);
    }
}
...
fn test_user_keyword_filename_vs_content_matching() {
    ...
    println!("Search result: {} lines, {} bytes, {} tokens", 
...
fn test_search_determinism_with_multiple_conditions() {
    ...
    for (name, config_flags) in search_configs {
        ...
        for i in 1..=10 {
            ...
            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                panic!("Command failed on iteration {} with {}: {}", i, name, stderr);
            }
        ...
        if !inconsistent {
            println!("✅ {} configuration is consistent: {} lines, {} bytes, {} tokens", 
...
fn test_search_determinism_concurrent_execution() {
    ...
    for (thread_id, result) in &results {
        println!("  Thread {}: {} lines, {} bytes, {} tokens", 
    ...
    for (thread_id, result) in results.iter().skip(1) {
        if !results_equal(first_result, result) {
            ...
            println!("  Thread 0: {} lines, {} bytes, {} tokens", 
                     first_result.line_count, first_result.byte_count, first_result.token_count);
            println!("  Thread {}: {} lines, {} bytes, {} tokens", 
                     thread_id, result.line_count, result.byte_count, result.token_count);
            inconsistent = true;
    ...
    if !inconsistent {
        println!("✅ Concurrent execution is consistent: {} lines, {} bytes, {} tokens", 
                 first_result.line_count, first_result.byte_count, first_result.token_count);
    }
