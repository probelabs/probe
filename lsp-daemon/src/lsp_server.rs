...
impl LspServer {
    ...
    pub async fn implementation(&self, file_path: &Path, line: u32, column: u32) -> Result<Value> {
        ...
        self.send_request("textDocument/implementation", params, request_id)
            .await?;
        let response = self
            .wait_for_response(request_id, Duration::from_secs(30))
            .await?;
        ...
        if let Some(error) = response.get("error") {
            return Err(anyhow!("Implementation request failed: {:?}", error));
        }
    ...
    pub async fn type_definition(&self, file_path: &Path, line: u32, column: u32) -> Result<Value> {
        ...
        self.send_request("textDocument/typeDefinition", params, request_id)
            .await?;
        let response = self
            .wait_for_response(request_id, Duration::from_secs(30))
            .await?;
        ...
        if let Some(error) = response.get("error") {
            return Err(anyhow!("Type definition request failed: {:?}", error));
        }
