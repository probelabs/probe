...
use probe_code::ranking::{rank_documents, rank_documents_simd, rank_documents_simd_simple, RankingParams};
use probe_code::simd_ranking::{SparseVector, SparseDocumentMatrix};
...
fn generate_code_documents(num_docs: usize) -> Vec<String> {
    ...
    let functions = ["process", "handle", "execute", "compute", "calculate", "transform", "validate"];
    let objects = ["data", "input", "output", "result", "value", "parameter", "config"];
    let operations = ["load", "save", "parse", "format", "convert", "merge", "filter"];
...
fn bench_sparse_vector_ops(c: &mut Criterion) {
    ...
    for &size in &sizes {
        ...
        group.bench_with_input(
            ...
            &(&sparse1, &sparse2),
            |b, (v1, v2)| {
                b.iter(|| black_box(v1.dot_product(v2)))
            },
        );
        ...
        group.bench_with_input(
            ...
            &(&sparse1, &sparse2),
            |b, (v1, v2)| {
                b.iter(|| black_box(v1.intersect_indices(v2)))
            },
        );
    }
...
fn bench_ranking_synthetic(c: &mut Criterion) {
    ...
    for &num_docs in &doc_counts {
        ...
        let queries = ["function process data", "class method handle", "variable input output load"];
        ...
        for query in queries {
            ...
            group.bench_with_input(
                ...
                &params,
                |b, params| {
                    b.iter(|| black_box(rank_documents(params)))
                },
            );
            ...
            group.bench_with_input(
                ...
                &params,
                |b, params| {
                    b.iter(|| black_box(rank_documents_simd_simple(params)))
                },
            );
        }
    }
