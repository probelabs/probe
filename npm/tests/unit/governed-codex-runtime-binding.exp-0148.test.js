...
agent.answerGoverned('x', { schema, resultIdentity: 'probe.governed-result-identity/v1' });`;
  ...
  await t.test('EXP-0152 T15 frozen files and legacy function/declaration regions remain exact', async () => {
    ...
    assert.equal(sha(await read('../../src/agent/engines/codex.js')), 'c23be8f4f46c69302e6b2d16148b54d19e208a86726ddfe0592c468fce15f574'); assert.equal(sha(await read('../../src/agent/schemaUtils.js')), '24332877e019ef29311f03ce9b63e61925c25fd7f83f5dd442b22dc68c60f6e9'); assert.equal(sha(await read('../../src/agent/engines/governed-codex-profile.js')), 'b0824ae50bb26a4c189e8224392cedbc8116b092a16baa199acd97aadcf7ced9');
