...
impl BertReranker {
    pub async fn new(model_name: &str) -> Result<Self> {
        ...
        if debug_mode {
            println!("DEBUG: Model config loaded - max_length: {}, hidden_size: {}", max_length, config.hidden_size);
        }
        ...
        let classifier = match linear(config.hidden_size, 1, vb.pp("classifier")) {
            ...
            Err(_) => {
                ...
                if debug_mode {
                    println!("DEBUG: Failed to load classifier from 'classifier' path, trying 'pooler'");
                }
    ...
    fn score_pair(&self, query: &str, document: &str) -> Result<f32> {
        ...
        let debug_mode = std::env::var("DEBUG").unwrap_or_default() == "1";
        if debug_mode {
            println!("DEBUG: Raw BERT score for query '{}' (first 50 chars): {:.6}", 
                &query[..query.len().min(50)], score);
        }
...
impl BertReranker {
    ...
    pub async fn rerank(&self, _query: &str, _documents: &[&str]) -> Result<Vec<(usize, f32)>, anyhow::Error> {
        Err(anyhow::anyhow!(
            "BERT reranker is not available. Build with --features bert-reranker to enable."
        ))
    }
...
impl ParallelBertReranker {
    pub async fn new(model_name: &str, num_threads: Option<usize>) -> Result<Self> {
        ...
        if debug_mode {
            println!("DEBUG: Creating parallel BERT reranker with {} engines", num_threads);
        }
        
        let mut engines = Vec::new();
        for i in 0..num_threads {
            if debug_mode {
                println!("DEBUG: Loading BERT model for engine {} of {}", i + 1, num_threads);
            }
            let engine = BertInferenceEngine::new(model_name).await?;
            engines.push(std::sync::Arc::new(parking_lot::Mutex::new(engine)));
        }
        
        Ok(Self { engines, num_threads })
    }
    ...
    pub fn rerank_parallel(&self, query: &str, documents: &[&str]) -> Result<Vec<(usize, f32)>> {
        ...
        if debug_mode {
            println!("DEBUG: Starting BERT reranking for {} documents with {} engines (num_threads={})", 
                    total_docs, self.engines.len(), self.num_threads);
        }
        ...
        let progress_reporter = if let (Some(ref processed_count), Some(ref processing_complete)) = (&processed_count, &processing_complete) {
                ...
                loop {
                    ...
                    if processing_complete_clone.load(Ordering::Acquire) {
                        ...
                        if current > last_reported && current > 0 {
                            ...
                            println!("DEBUG: BERT reranking progress: {}/{} documents processed ({:.1} docs/sec)", 
                                    current, total_docs, rate);
                        }
                        break;
                    }
        ...
        if debug_mode {
            eprintln!("DEBUG: About to start parallel processing of {} documents", total_docs);
        }
        
        // Create chunks for parallel processing - each chunk is processed by a dedicated engine
        let chunk_size = (total_docs + self.engines.len() - 1) / self.engines.len();
        ...
        if debug_mode {
            println!("DEBUG: Created {} chunks of size ~{}", chunks.len(), chunk_size);
        }
        ...
        let results: Result<Vec<Vec<(usize, f32)>>> = chunks
            ...
            .map(|(chunk_idx, chunk)| {
                ...
                if debug_mode {
                    println!("DEBUG: Thread {} processing chunk {} with {} documents using engine {}", 
                             chunk_idx, chunk_idx, chunk.len(), engine_idx);
                }
                ...
                for (doc_idx, document) in chunk {
                    let score = engine_guard.score_pair(&query, document)
                    ...
                    if let Some(ref count) = processed_count {
                        ...
                        if debug_mode && new_count % 10 == 0 {
                            eprintln!("DEBUG: Processed {} documents so far", new_count);
                        }
                    }
                }
                ...
                if debug_mode {
                    println!("DEBUG: Chunk {} completed processing {} documents", chunk_idx, chunk_results.len());
                }
        ...
        if debug_mode {
            ...
            println!("DEBUG: BERT reranking completed: {} documents in {:.2}s ({:.1} docs/sec)", 
                    total_docs, elapsed.as_secs_f64(), rate);
        }
        ...
        if debug_mode {
            println!("DEBUG: Parallel processing complete, {} results sorted", all_scores.len());
        }
