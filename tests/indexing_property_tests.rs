...
fn manager_config_strategy() -> impl Strategy<Value = ManagerConfig> {
    ...
    ).prop_map(|(workers, memory, threshold, queue_size, file_size, batch_size)| {
        ManagerConfig {
            max_workers: workers,
...
proptest! {
    ...
    fn prop_manager_file_discovery(
        ...
        rt.block_on(async {
            ...
            let discovery_result = timeout(Duration::from_secs(10), async {
                loop {
                    ...
                    if progress.total_files > 0 {
                        return Ok(progress.total_files);
                    }
            ...
            if let Ok(Ok(discovered_files)) = discovery_result {
