...
3.  **Start with \`search\`:**
    *   **Use a well-formed question:** Provide a clear, natural-language question about the codebase. Avoid single-token or keyword-only queries; the tool will translate your question into effective searches.
...
*   \`search\`
    ...
    *   **Syntax:** \`query\` (well-formed question about the code), \`path\` (Mandatory: \`"."\`, \`"path/to/dir"\`, \`"path/to/file.ext"\`, \`"go:pkg"\`, \`"js:npm_module"\`, \`"rust:crate"\`), \`exact\` (Optional: Set to \`true\` for case-insensitive exact matching without tokenization).
...
*   **Example 1: Finding a Specific Function Definition**
    ...
    *   Probe Action 1: \`search\` query: \`How is the total calculated in payments?\`, path: \`"src/payments"\` (Targeted search in the likely directory)
...
*   **Example 2: Investigating Initialization**
    ...
    *   Probe Action 1: \`search\` query: \`How is Redis configured and initialized?\`, path: \`"."\`
...
*   **Example 3: Understanding Usage of a Dependency Feature**
    ...
    *   Probe Action 1: \`search\` query: \`Where is createAnthropic used in this project?\`, path: \`"."\` (Search project code for usage)
    *   (Analysis: Find usage in \`src/ai/providers.ts\`. Want to understand the library function itself better.)
    *   Probe Action 2: \`search\` query: \`Where is createAnthropic defined in the dependency?\`, path: \`"js:@ai-sdk/anthropic"\` (Search within the specific dependency)
...
*   **Example 4: Exploring Error Handling Patterns**
    ...
    *   Probe Action 1: \`search\` query: \`How does error handling work in the Go services?\`, path: \`"service/"\` (Focus on service directories)
    ...
    *   Probe Action 2: \`search\` query: \`Where is pkg/errors imported in service code?\`, path: \`"service/"\` (Check where a potential custom error package is used)
