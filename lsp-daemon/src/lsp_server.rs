...
impl LspServer {
    pub fn spawn(config: &LspServerConfig) -> Result<Self> {
        ...
        Ok(Self {
            ...
            stdout: Arc::new(Mutex::new(Box::new(BufReader::new(stdout)) as Box<dyn BufRead + Send>)),
                    ...
                    if let Some(method) = msg.get("method").and_then(|m| m.as_str()) {
                        ...
                        if method == "window/workDoneProgress/create" {
                            if let Some(id) = msg.get("id") {
                                self.send_response(id.as_i64().unwrap_or(0), json!(null)).await?;
                            }
                        }
                    }
                }
    ...
    async fn wait_for_response(&self, id: i64, timeout_duration: Duration) -> Result<Value> {
        ...
        while start.elapsed() < timeout_duration {
            if let Some(msg) = self.read_message_timeout(Duration::from_millis(100)).await? {
    ...
    pub async fn call_hierarchy(&self, file_path: &Path, line: u32, column: u32) -> Result<Value> {
        ...
        self.send_request("textDocument/prepareCallHierarchy", params, request_id).await?;
        let response = self.wait_for_response(request_id, Duration::from_secs(5)).await?;
        
        if let Some(error) = response.get("error") {
            return Err(anyhow!("Call hierarchy prepare failed: {:?}", error));
        }
        
        let items = response["result"].as_array()
        ...
        // Get incoming calls
        let request_id = self.next_request_id().await;
        self.send_request("callHierarchy/incomingCalls", json!({ "item": item }), request_id).await?;
        let incoming_response = self.wait_for_response(request_id, Duration::from_secs(5)).await?;
        
        // Get outgoing calls
        let request_id = self.next_request_id().await;
        self.send_request("callHierarchy/outgoingCalls", json!({ "item": item }), request_id).await?;
        let outgoing_response = self.wait_for_response(request_id, Duration::from_secs(5)).await?;
    ...
    pub async fn shutdown(&self) -> Result<()> {
        let request_id = self.next_request_id().await;
        self.send_request("shutdown", json!(null), request_id).await?;
        
        // Wait for shutdown response
        let _ = self.wait_for_response(request_id, Duration::from_secs(2)).await;
