...
pub fn process_file_for_extraction(
    ...
    if let (Some(start), Some(end)) = (start_line, end_line) {
        if debug_mode {
            println!("[DEBUG] Extracting line range: {}-{} (with AST merging)", start, end);
        }
        ...
        let code_blocks_result = parse_file_for_code_blocks(&content, file_extension(path), &needed_lines, allow_tests, None);
    ...
    // Single line extraction
        ...
        match parse_file_for_code_blocks(&content, file_extension(path), &needed_lines, allow_tests, None) {
            Ok(blocks) if !blocks.is_empty() => {
        ...
        let code_blocks_result = parse_file_for_code_blocks(&content, file_extension(path), lines_set, allow_tests, None);
        ...
        match code_blocks_result {
            ...
            _ => {
                ...
                let start = if min_line <= context_lines { 1 } else { min_line - context_lines };
