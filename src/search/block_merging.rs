...
pub fn merge_ranked_blocks(results: Vec<SearchResult>, threshold: Option<usize>) -> Vec<SearchResult> {
    ...
    if debug_mode {
        println!("DEBUG: Starting post-rank merging of {} results with threshold {}", results.len(), threshold);
    }
    ...
    for result in results {
        file_blocks.entry(result.file.clone())
    ...
    for (file_path, mut blocks) in file_blocks {
        if debug_mode {
            println!("DEBUG: Processing {} blocks from file: {}", blocks.len(), file_path);
        }
        ...
        while i < blocks.len() {
            ...
            while j < blocks.len() {
                ...
                if should_merge_blocks(&current_block, &blocks[j], threshold) {
                    ...
                    if debug_mode {
                        println!("DEBUG: Merging blocks - current: {}-{}, next: {}-{}", 
                                 current_block.lines.0, current_block.lines.1, 
                                 next_block.lines.0, next_block.lines.1);
                    }
    ...
    if debug_mode {
        println!("DEBUG: Post-rank merging complete. Merged {} blocks into {} blocks", 
                 original_count, merged_results.len());
    }
...
pub fn should_merge_blocks(block1: &SearchResult, block2: &SearchResult, threshold: usize) -> bool {
    ...
    let effective_threshold = if is_function_like(&block1.node_type) && is_function_like(&block2.node_type) {
    ...
    let distance = if start2 > end1 { start2 - end1 } else if start1 > end2 { start1 - end2 } else { 0 };
    ...
    if debug_mode {
        ...
        println!("DEBUG: Should merge: {} (distance: {}, effective_threshold: {})",
...
fn merge_block_content(block1: &SearchResult, block2: &SearchResult) -> String {
    ...
    if debug_mode {
        println!("DEBUG: Current working directory: {:?}", std::env::current_dir().unwrap_or_else(|_| PathBuf::from("unknown")));
        println!("DEBUG: Attempting to read file: {:?}", file_path.canonicalize().unwrap_or_else(|_| PathBuf::from(file_path)));
        ...
        if let Some(line_content) = line_map.get(&current_line) {
            ...
            if gap_size < 10 {
                if file_content_available {
                    if debug_mode {
                        println!("DEBUG: Attempting to fill small gap from line {} to {} from file {}", gap_start, gap_end, block1.file);
                    }
                    ...
                    if let Ok(file) = file_result {
                        ...
                        for line_result in reader.lines() {
                            if let Ok(line_content) = line_result {
                                if current_line_in_file >= gap_start && current_line_in_file <= gap_end {
                                    if debug_mode {
                                        println!("DEBUG: Including line {}: {}", current_line_in_file, line_content);
                                    }
                        ...
                        if !lines_read.is_empty() {
                            if debug_mode {
                                println!("DEBUG: Successfully read {} lines for gap", lines_read.len());
                            }
                ...
                merged_lines.push(format!("... lines {}-{} should be included ...", gap_start, gap_end));
            } else {
...
fn merge_scores(block1: &SearchResult, block2: &SearchResult) -> (Option<f64>, Option<f64>, Option<f64>, Option<f64>) {
...
fn merge_term_statistics(block1: &SearchResult, block2: &SearchResult) -> (Option<usize>, Option<usize>) {
