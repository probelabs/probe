...
pub fn test_simd_implementation() {
    ...
    if let Some(simd_dot) = f32::dot(&a, &b) {
        ...
        println!("SimSIMD is working: {}", (simd_dot - expected_dot).abs() < 0.001);
    } else {
    ...
    println!("Vector 1: indices={:?}, values={:?}", sparse1.indices, sparse1.values);
    println!("Vector 2: indices={:?}, values={:?}", sparse2.indices, sparse2.values);
    ...
    assert!((dot_product - 23.0).abs() < 0.001, "SIMD dot product should be 23.0, got {}", dot_product);
