...
"#;
        ...
        assert_eq!(parsed_results[0].3, Some("MyStruct.new".to_string()), 
...
"#;
        ...
        if let Ok(search_result) = result {
            ...
            assert!(output.contains("pub fn new() -> Self"), "Should extract new method");
        } else {
...
"#;
        ...
        match result {
            Ok(search_result) => {
                println!("✅ End-to-end extraction successful: {}", search_result.code);
            }
            Err(_) => {
