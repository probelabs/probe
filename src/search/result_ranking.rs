...
pub fn rank_search_results(results: &mut [SearchResult], queries: &[String], reranker: &str) {
    ...
    // Use SIMD-optimized ranking if enabled via environment variable
    let use_simd = std::env::var("USE_SIMD_RANKING").unwrap_or_default() == "1";
    let ranked_indices = if use_simd {
        ...
        if debug_mode && std::env::var("USE_SIMD_RANKING").is_ok() {
