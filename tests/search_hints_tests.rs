...
fn test_filter_extraction_and_ast_simplification() {
    ...
    assert!(filters.file_patterns.contains(&"src/**/*.rs".to_string()) ||
    ...
    match ast {
        elastic_query::Expr::Or(left, right) => {
            ...
            match (left.as_ref(), right.as_ref()) {
                (elastic_query::Expr::Term { keywords: left_kw, .. }, 
                 elastic_query::Expr::Term { keywords: right_kw, .. }) => {
                    let all_keywords: Vec<String> = left_kw.iter()
                        .chain(right_kw.iter())
                        .cloned()
                        .collect();
                    assert!(all_keywords.contains(&"error".to_string()) || 
        ...
        elastic_query::Expr::Term { keywords, .. } => {
            ...
            assert!(keywords.contains(&"error".to_string()) || 
                   ...
                   keywords.iter().any(|k| k.contains("error") || k.contains("handling")));
        }
        _ => {
...
fn test_multiple_extensions() {
    let mut filters = SearchFilters::new();
    filters.add_filter("ext", vec!["rs".to_string(), "js".to_string(), "ts".to_string()]);
