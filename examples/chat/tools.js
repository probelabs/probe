...
import { searchTool as probeSearchTool, queryTool as probeQueryTool, extractTool as probeExtractTool } from '@buger/probe';
...
// Get the native tools from the probe package
const nativeSearchTool = probeSearchTool(configOptions);
const nativeQueryTool = probeQueryTool(configOptions);
const nativeExtractTool = probeExtractTool(configOptions);
...
/**
 * Wrapper for the search tool that maintains the same interface
 * but uses the native probe package tool internally
 */
export const searchTool = {
  name: 'search',
  description: nativeSearchTool.description || 'Search code in the repository using Elasticsearch-like query syntax. Use this tool to find relevant code snippets based on keywords or patterns.',
  parameters: {
    type: 'object',
    properties: {
      query: {
  ...
  execute: async ({ query, path, exact, allow_tests }) => {
    try {
      // Try to use the native tool first
      try {
        const result = await nativeSearchTool.execute({
          query,
          path: path || (process.env.ALLOWED_FOLDERS ? process.env.ALLOWED_FOLDERS.split(',')[0].trim() : '.'),
          exact: exact || false,
          allowTests: allow_tests || false,
          maxTokens: 40000
        });
        return result;
      } catch (nativeError) {
        console.warn(`Native search tool failed, falling back to CLI: ${nativeError.message}`);

        // Fall back to CLI if native tool fails
...
/**
 * Wrapper for the query tool that maintains the same interface
 * but uses the native probe package tool internally
 */
export const queryTool = {
  name: 'query',
  description: nativeQueryTool.description || 'Search code using ast-grep structural pattern matching. Use this tool when you need to find specific code structures like functions, classes, or methods.',
  parameters: {
    type: 'object',
    properties: {
      pattern: {
  ...
  execute: async ({ pattern, path, language, allow_tests }) => {
    try {
      // Try to use the native tool first
      try {
        const result = await nativeQueryTool.execute({
          pattern,
          path: path || (process.env.ALLOWED_FOLDERS ? process.env.ALLOWED_FOLDERS.split(',')[0].trim() : '.'),
        ...
        console.warn(`Native query tool failed, falling back to CLI: ${nativeError.message}`);
...
/**
 * Wrapper for the extract tool that maintains the same interface
 * but uses the native probe package tool internally
 */
export const extractTool = {
  name: 'extract',
  description: nativeExtractTool.description || 'Extract code blocks from files based on file paths and optional line numbers. Use this tool after finding relevant files with the search tool to see complete context.',
  parameters: {
    type: 'object',
    properties: {
      file_path: {
  ...
  execute: async ({ file_path, line, end_line, allow_tests, context_lines, format }) => {
    try {
      ...
      // Try to use the native tool first
      try {
        const result = await nativeExtractTool.execute({
          files: [fullFilePath],
          line,
          endLine: end_line,
          allowTests: allow_tests || false,
          contextLines: context_lines || 10,
          format: format || 'plain'
        });
        return result;
      } catch (nativeError) {
        console.warn(`Native extract tool failed, falling back to CLI: ${nativeError.message}`);
