...
export async function listFilesByLevel(options) {
    const { directory, maxFiles = 100, respectGitignore = true } = options;
    ...
    while (queue.length > 0 && result.length < maxFiles) {
        ...
        try {
            ...
            for (const subdir of dirs) {
                ...
                if (shouldIgnore(relativeSubdirPath, ignorePatterns))
                    continue;
                // Skip node_modules and .git directories
                if (subdir.name === 'node_modules' || subdir.name === '.git')
                    continue;
...
function shouldIgnore(filePath, ignorePatterns) {
    ...
    for (const pattern of ignorePatterns) {
        ...
        if (pattern.endsWith('*') && filePath.startsWith(pattern.substring(0, pattern.length - 1)))
            return true;
    }
    return false;
}
