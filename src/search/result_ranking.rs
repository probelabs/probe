...
pub fn rank_search_results(results: &mut [SearchResult], queries: &[String], reranker: &str) {
    ...
    let _node_type = results.first().map(|r| r.node_type.as_str());
    ...
    // Store original document indices and their BM25 scores for later use
    let score_update_start = Instant::now();
    let mut doc_scores: Vec<(usize, f64)> = Vec::new();
    ...
    for (rank_index, (original_index, bm25_score)) in ranked_indices.iter().enumerate() {
        doc_scores.push((*original_index, *bm25_score));

        if let Some(result) = results.get_mut(*original_index) {
            result.rank = Some(rank_index + 1); // 1-based rank
            result.score = Some(*bm25_score); // Use BM25 as the main score
            result.tfidf_score = Some(0.0); // Set TF-IDF to 0 since we're only using BM25
            result.bm25_score = Some(*bm25_score);
            result.new_score = Some(*bm25_score); // Use BM25 for new_score as well
        }
    }
    ...
    // Only BM25 ranking is supported
    if debug_mode {
